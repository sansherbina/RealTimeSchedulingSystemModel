...
public class ASchedulingAlgorithm implements ISchedulingAlgorithm {
	...
	public TaskScheduling scheduleTask(ExecutionCostMatrix executionCostMatrix)
		...
		for (int i = 0; i < ITEM_COUNT_FOR_GENERATING_RANDOM_SCHEDULING; i++) {
			randomGeneratedScheduling = TaskScheduling
					.generateRandomScheduling(executionCostMatrix);
			if (!randomGeneratedScheduling
		...
		openList.insert(new TaskScheduling(executionCostMatrix
				.getExecutionCostMatrix().length));
		...
		while (true) {
			...
			openList.insert(taskScheduling
					.generateChildren(executionCostMatrix));
		}
